# Contact: Tereza Novotná

## Quick reference

| Field | Value |
|-------|-------|
| Name | Tereza Novotná |
| Role | Senior Data Scientist (L4) @ Data - Apify's first data scientist |
| Team | Data — direct report to Vojta |
| Tier | 2 |
| Email | tereza.novotna@apify.com (confirm); personal t3r3z.novotna@gmail.com seen on invites |
| Slack | TBD |
| Timezone | Europe/Prague (confirm) |
| Start date | 2026-06-15 |
| Buddy | Markéta Trunkátová |

## Why she matters

- The data team's first data scientist - her hire effectively closes the `hiring_and_ladder` goal, and her ramp is also a function-definition exercise (what "data science at Apify" means).
- First project: optimising retention vs recurring revenue.

## Relationship context

- Joined 2026-06-15. Onboarding plan: [Onboarding - Tereza Novotná](https://app.notion.com/p/37ff39950a228168a11fe2081b2993a8). Company onboarding wrapped Mon ~12:00 (Kristýna + Laura), team lunch followed.

## Communication style

- To be filled from 1:1s.

## Talking points for next interaction

- Welcome 1:1: frame the "you're defining the DS function, not inheriting it" mission and the no-pressure learning loop.
- Co-write her "Data Science at Apify" role definition and 90-day goals.
- Scope the retention-vs-recurring-revenue project (target metric, recurring-revenue definition) with Bára, Martina, and Markéta.

## Last interaction

- Date: 2026-06-15 (start)
- Channel: In person (Day 1)
- Follow-up needed: Daily 10-min check-in through week 1; confirm access landed; set the day-30 review.
